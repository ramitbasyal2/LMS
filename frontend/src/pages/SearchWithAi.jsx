...
const SearchWithAi = () => {
    const startSound = new Audio(start) //for mic start sound
    const navigate = useNavigate()
    const [input, setInput] = useState("");
    const [recommendations, setRecommendations] = useState([])
    const [listening, setListening] = useState(false)

    // to convert message to speech
    ...
    const handleSearch = async () => {
        setListening(true)
         ...
         recognition.start()
         startSound.play();//play the mic sound
         recognition.onresult = async (e) => {
            //  console.log(e)
            const transcript = e.results[0][0].transcript.trim()
            setInput(transcript)
            await handleRecommendation(transcript)//passing the query
         }
    }
       ...
       <div className='bg-white shadow-xl rounded-3xl p-6 sm:p-8 w-full 
        max-w-2xl text-center relative'>
            ...
            <h1 className='text-2xl sm:text-3xl font-bold text-gray-600 mb-6 
             flex items-center justify-center gap-2 '> <img src={img} className='w-8 h-8 
             sm:w-[30px] sm:h-[30px]' alt="" /> Search with  
               <span className='text-[#CB99C7]'>AI</span> </h1>

               <div className='flex items-center bg-gray-700 rounded-full overflow-hidden shadow-lg 
                     relative w-full' >
                        <input onChange={(e)=> setInput(e.target.value)} value={input} type="text" className='flex-grow px-4 py-3 
                        bg-transparent text-white placeholder-gray-40 
                        focus:outline-none text-sm sm:text-base' placeholder='What do you want to learn? (e.g. AI, MERN, Cloud...)' />
 
                         { input &&
                            <button className='absolute right-14 sm;right-16 bg-white rounded-full'><img src={img} className='w-10 
                         h-10 p-2 rounded-full' onClick={()=> handleRecommendation(input)} alt="" /></button>}
                        <button onClick={handleSearch} className='absolute right-2 bg-white rounded-full w-10 h-10 flex items-center 
                        justify-center'><RiMicAiFill className='w-5 h-5 text-[#cb87c5]' /></button>                 
               </div>
       </div>
                      ...
                      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8  ' >
                           {
                            recommendations?.map((course,index) => (
                                <div onClick={()=> navigate(`/viewcourse/${course._id}`)} key={index} className='bg-white text-black p-5 rounded-xl shadow-md
